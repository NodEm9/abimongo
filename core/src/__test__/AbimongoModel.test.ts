...
describe('AbimongoModel', () => {
	...
	it('should insert a document when valid data is provided', async () => {
		...
		jest.spyOn(AbimongoClient, 'getDatabase').mockResolvedValue(mockDb as unknown as Db);
	...
	it('should initialize with tenant database and collection', async () => {
		...
		jest.spyOn(AbimongoClient, 'getDatabase').mockResolvedValue(db.tenantDB as unknown as Db);
	...
	describe('findOne', () => {
		...
		it('should return a document when found', async () => {
			...
			jest.spyOn(AbimongoClient, 'getDatabase').mockResolvedValue(mockDb.db() as unknown as Db);
			await model.init()
	...
	describe('aggregate', () => {
		it('should perform aggregation on the collection', async () => {
			...
			mockCollection.aggregate(mockCursor);
			new EventEmitter().emit('aggregate', mockCursor.bufferedCount());
			mockCursor.toArray()
