...
export async function generateProject(choices: ProjectChoices) {
  const { projectType, projectName, useAbimongo, useTypeScript, includeLogger } = choices;
  ...
  switch (projectType) {
    case 'MERN Stack':
      return await handleMERNStack(projectName, { useTypeScript, useAbimongo, includeLogger });;
    case 'Next.js App':
      return await handleNextApp(projectName, { useTypeScript, useAbimongo, includeLogger });
    case 'REST API':
      return await handleRestAPI(projectName, { useTypeScript, useAbimongo, includeLogger });
    case 'GraphQL API':
