...
export class AbimongoClient implements AbimongoClientConfig {
	...
	private static defaultUri: string = process.env.MONGO_URI || `mongodb://127.0.0.1:27017`;
	...
	static async getDatabase(tenantId: string, uri: string): Promise<Db> {
		...
		if (!this.tenantDBs.has(tenantId)) {
			const client = await _abimongo.connectDb(uri, { dbName: tenantId })
			await client._client?.connect()

			// If not, create a new database connection for the tenant
			if (client._client) {
				this.instances.set(tenantId, client._client.db(tenantId));
			} else {
				throw new Error('MongoClient instance is undefined.');
			}
			this.tenantDBs.set(tenantId, client._client.db(tenantId));
		}
		// Return the cached tenant database
